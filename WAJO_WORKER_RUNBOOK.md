...
2026-06-11 09:08 JST に、名刺入口の Yoom フロー `【本番】名刺管理DB → Notion Worker起動` は `トリガーOFF` にした。

以後、名刺管理の正本は Worker が読む `名刺管理 DB` / data source `0634132a-c6cf-458d-b63c-b357489f227e` に一本化する。旧Yoom監視側 `📇 名刺管理` / data source `3634d017-81e7-8177-b796-000b96b5d885` は、復活しても正本に戻さない。必要な場合は中身救出・旧設定確認用の退避対象として扱う。

名刺の重複判定、企業作成、既存企業紐づけ、3C返却は Worker 側に寄せる。Yoom は名刺入口の起動経路として使わない。
...
- `名刺管理 DB`: `0634132a-c6cf-458d-b63c-b357489f227e`
- 本番Worker env: `BUSINESS_CARD_DATA_SOURCE_ID=0634132a-c6cf-458d-b63c-b357489f227e`

通常の処理確認:

```bash
NOTION_KEYRING=0 /Users/isibasidaisuke/.local/bin/ntn workers exec processPendingBusinessCards \
  --worker-id 019e452d-22e7-7de1-b5ea-432a297bb478 \
  -d '{"limit":5,"dryRun":true}'
```

書き込み実行は、dry-run結果を見てから `dryRun:false` にする。空行や会社名・メール・電話・名刺画像がない行は `skipped` になるため、先にNotion側で入力を補う。

旧Yoomフローは再ONにしない。もし名刺入口の自動起動を復活させる場合は、Yoomではなく Notion Worker の sync / scheduled capability / Notionボタン直起動のいずれかで設計し直す。
...
パーソナルプランでは `カスタムコネクト` がプラン制限になり、フローボットが実行できないことがある。2026-06-11 時点でも画面上にプラン制限警告が出ていたため、名刺入口からYoomを外した。
...
- 承認対象は `ノルマ申請` と `日報` だけに限定する。
- 案件化、担当割り当て、成約、失注、タスク化には承認フローを置かない。
- `processClosingReportWebhook` は、案件管理DBを `🏆 成約`、案件の `成約日` を当日、成約報告DBの成約レコード作成、営業マンパフォーマンスDBの月次反映まで即時実行する。
- 成約報告DBへ新規作成する成約レコードには `承認ステータス=成約` を書き込まない。
- 成約差し戻し、成約取り消し、案件差し戻し、案件取り消し、失注申請、失注承認、失注差し戻しのWebhookは退役。
- 失注は `processProjectLostWebhook` で、理由必須の即時失注として扱う。
- 営業貢献ログは `承認ステータス` ではなく、`評価反映状態` と根拠コメントの有無で活動ログ化する。
...
2026-05-26 に採用した「報告即時反映 + マネージャー後追い管理」は、2026-06-11 に退役しました。現行は「報告即時反映 + 公開通知 + 履歴」であり、成約側の後追い管理ゲートは置きません。

2026-05-27 に月次反映先を再整理しました。ノルマ申請DBは「目標申請の原本」とし、成約報告の実績反映先は営業マンパフォーマンスDBの月次成績ビューに戻します。`processClosingReportWebhook` は成約報告DBへ成約1件を作ったあと、営業マンパフォーマンスDBから `期間種別 = 月次`、`対象営業ユーザー = 案件の担当営業ユーザー`、`開始日/対象期間 = 当月` のレコードを探し、その `関連成約` に成約報告を追記します。該当月次レコードがなければ `YYYY年M月 月次成績` を自動作成します。担当営業ユーザーが未設定の場合のみ、クリックしたユーザーを補助的に使います。
...
同日に追加した案件管理DBの後追い管理アクション導線は、2026-06-11 に退役しました。今後、案件管理DBの通常導線に後追い管理アクションボタンは置きません。
...
- 案件失注: `processProjectLostWebhook`
...
- 成約差し戻し、成約取り消し、案件差し戻し、案件取り消しは置かない。
- `失注にする` は案件管理DBに置く。理由必須で即時失注にし、公開通知する。
...
2026-05-30 に、問い合わせフェーズと案件フェーズは営業個人が簡単に手入力で変えられない運用へ寄せる方針にしました。営業画面の `問い合わせフェーズ` / `案件フェーズ` / `ステータス` は原則として現在地の表示にし、状態変更は `担当になる`、`案件化する`、`成約を報告する`、`失注にする` などのNotionボタンまたはWorker経由へ集約します。

Worker実装では、`成約`、`失注`、`完了` のように数字・評価へ影響する状態を直接書き換える処理に、条件チェック、処理者、処理日、前フェーズ、メモの記録をできるだけ持たせます。Notion単体では「DBは編集可だがステータス列だけ編集不可」の制御が弱いため、次に問い合わせ入口、Yoomail、案件フェーズ、成約/失注フローを触る時は、手入力ステータスを前提にしない設計を優先します。
...
問い合わせ段階も案件化後も、失注は「失注理由必須 + 全体/管理者へのアナウンス」で運用します。誰がどの問い合わせ/案件を何の理由で失注にしたかを流し、えり好みや雑な見切りを可視化します。
...
2026-05-31 に追加した失注確定済み案件向けのマネージャー操作は、2026-06-11 に退役しました。現行UIには、失注の後追い管理ボタンを置きません。
