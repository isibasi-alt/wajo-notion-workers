...
- スレッド開始時、AIエージェント自身が、今から触る作業に対応するプロジェクトフォルダを Obsidian Vault の `20_Project` 配下から先に探して読む。
...
- 先に `20_Project` 直下の候補フォルダ名を確認し、ユーザー発話・作業名・リポジトリ名・関連DB名・ファイル名・Notion/Obsidian上の名称と照合する。
- 候補フォルダが1つに絞れた場合だけ、そのフォルダを対象にする。
- 候補フォルダが複数ある、または1つに絞れない場合は、作業を始めずに `候補フォルダ`、`選べない理由`、`ユーザーに確認する質問` を出して止まる。
- 特定プロジェクト名を固定しない。`名刺一枚` など目についたフォルダへ飛びつかない。
...
- 作業前の最初の返答で、`候補フォルダ`、`選んだフォルダ`、`読むファイル`、`読めなかったファイル` を明示する。
- 実際に読んでいない場合は、読んだと言わない。入口ファイルにルールがあることと、新スレッドが実際に読んだことを混ぜない。
