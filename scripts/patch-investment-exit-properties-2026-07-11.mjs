...
const PROPERTIES = {
	...
	出口時税金見込: { number: { format: "yen" } },
	和上買取コミット段階: {
		select: {
			options: [
				{ name: "出口想定", color: "gray" },
				{ name: "和上審査中", color: "yellow" },
				{ name: "和上コミット承認", color: "green" },
				{ name: "契約済み", color: "blue" },
			],
		},
	},
	和上買取コミット価格: { number: { format: "yen" } },
	和上買取コミット条件: { rich_text: {} },
	出口時残債: { number: { format: "yen" } },
	出口手取り: { number: { format: "yen" } },
	出口エクイティNPV: { number: { format: "yen" } },
	出口エクイティIRR: { number: { format: "number" } },
	"3年累計運用手取り": { number: { format: "yen" } },
	"3年総受取額": { number: { format: "yen" } },
	"3年投資差益": { number: { format: "yen" } },
	出口価格根拠: { rich_text: {} },
};
...
const VIEWS = [
	{
		...
		properties: [
			...
			"出口想定年数", "出口想定売却価格", "出口費用率", "出口時税金見込",
			"和上買取コミット段階", "和上買取コミット価格", "和上買取コミット条件",
			"即時償却適用", "ファイナンス状態", "ファイナンスメモ",
		],
	},
	{
		...
		properties: [
			...
			"出口価格根拠", "出口時残債", "出口手取り", "出口エクイティNPV", "出口エクイティIRR",
			"3年累計運用手取り", "3年総受取額", "3年投資差益",
