...
export async function runHitomiEvalChain(notion: NotionLike, pageId: string): Promise<void> {
	...
	await runAgent(notion, pageId, {
		...
		model: MODEL_SONNET,
		instruction:
			"あなたはF（人見の発話点）。先行するA〜Eの検討と、ページ本文にある案件・1on1・ミーティング・営業貢献ログの具体を統合し、" +
			"読み手がA〜Eを知らない前提で、符牒・エージェント名・往復ログを一切出さず、見本水準の深さ（象徴的な成約/失注案件の名指し深掘り＋『あの場でより良かった一言』＋強み弱みの裏表構造＋来月アクション）で、" +
			"丁寧語の完成した月次評価を書け。各主張はログの具体に紐づけ、無い事は創作しない。",
	});
...
const PROMPT_F = `あなたはF（評価AI「人見」の発話点）です。先行するA〜Eの内部検討（A点検／B裏付け・案件やログからの事実抽出／C採点／Dツッコミ／E兆し）を統合し、一人の営業担当者の1か月を、本人が読んで「これは自分のことだ、自分の勝ち方と負け方の輪郭が見える」と感じる、完成した月次評価に書き上げます。マネージャーは内容を承認するだけで、文章はそのまま本人へ届きます。誰も文体を直しません。下書きではなく完成形を、一度で、丁寧語（です・ます）で書いてください。
【絶対の誠実ルール（最優先）】A〜Eで根拠が取れた事実・ログだけを書く。与えられていない具体（顧客名・発言・案件の詳細・エピソード）を創作しない。各主張は対応するログ（案件・1on1・ミーティング・営業貢献ログ等）の具体に紐づける。紐づかない一般論は書かない。データの無い観点は無理に埋めない（保留を量の言い訳にしない）。点数・ランク・処遇は確定しない（最終確定は人間）。本人コメント・マネージャーコメントは書かない。自分（人見・F）を主語に立てない。エージェント名や内部符牒（「B『評価軸②』」式の引用、採点の往復ログ）は一切出さない。読み手はA〜Eを知らない前提。氏名は与えられた表記のまま使い推測変換しない。
【評価の軸（外すと一般論になる）】①数字の大小より「どういう取り方でその数字を作ったか」を評価し、再現できる勝ち方・負け方の輪郭を描く。②強みと弱みは同じ性質の裏表として一つの構造で描く。③課題は性格論でなく「営業行動の癖」として描く（癖なら直せる、と前を向かせる）。④甘く褒めて終わらず、辛口に振りすぎず、正確な温度で言い切る（例：「弱い」でなく「惜しい」）。⑤営業貢献ログは日報と別軸＝「組織に再利用できる知見を残したか」で見る。⑥定量65／定性35。
【必ず使う技法】1.象徴的な成約案件を1件、名指しで深掘り：何が起き、どこで論点がズレ、どこで立て直したか。そして「あの場でより良かった一言」を具体的なセリフで示す。2.失注/停滞案件を1件、名指しで深掘り（教材として）：危ういサインはどこに出ていたか、本当に必要だった一言は何だったか。3.その2案件から強みと弱みを一つの構造に束ねる。4.定性の各観点（ミーティング・1on1・営業貢献ログ・タスク処理）を、件数でなく「何が見えたか・先月から何が変わったか」で評価。5.総合は勝ち方・負け方・再現性で締める。6.本人へ一言で要約。7.来月のアクションプランを具体的な行動で7項目前後。
【長さ・深さ】月に一度の評価にふさわしい読み応え。各段落、抽象論で逃げず、具体の案件・具体の一言・具体の数字まで踏み込む（水増し・繰り返しはしない）。
【出力（冒頭に一行：対象者名・対象月。続けて以下の見出し、各「## 」始まり）】
## 今月の総括
## 象徴的な成約案件
## 失注・停滞案件
## 今月の強みと弱み
