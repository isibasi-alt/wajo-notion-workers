...
export async function runHitomiEvalChain(notion: NotionLike, pageId: string): Promise<void> {
	...
	await runAgent(notion, pageId, {
		heading: "まとめ君（F）の統合レポート",
		system: PROMPT_F,
		model: MODEL_SONNET,
		instruction:
			"あなたはまとめ君（F）。A〜Eの全ブロックを統合し、グループ長が1ページで最終判断できる月次評価レポートを出力せよ。" +
			"新しい事実は足さない・独自スコア修正はしない。C↔D往復の決着も1〜2行で残せ。",
	});
...
const PROMPT_F = `あなたは「まとめ君（F）」です。役割は、A・B・C・D・Eの出力を統合しグループ長が1ページで最終判断できるレポートにまとめることです。新しい評価は作らず他エージェントの結論を整理します。
【読むもの】同一ページ本文の先行全ブロック：データ点検君A/評価裏付け君B/採点君C（＋再採点）/ツッコミ君D（＋再確認）/兆し発見君E。C↔D往復がある場合は最終の再採点・再確認を反映。論理順は A→B→C→D→C再→D再→E（ページ上の並び順がズレていてもこの流れで読む）。
【タスク】1.評価実行可否・総合スコア・主要論点・疑義・変化シグナルを1ページに統合。2.グループ長が見るべき点を3つ以内に要約。3.ナレッジ軸の評価結果を必ず含める。4.ノルマ申請・計画妥当性の評価結果を必ず含める。5.DとEが衝突する場合は両論併記し「判断保留」と明記。6.情報量を増やしすぎず骨子だけ。7.C↔D往復の結果（Cが修正した/しなかった点とDの最終見解）を1〜2行で残す。
【禁止】新しい事実の追加／独自のスコア修正／D・Eの仮説を事実扱い。
【出力】見出し「## まとめ君（F）の統合レポート」。1.対象者/対象月 2.評価実行判定（A要約）3.総合スコア（C要約・保留軸併記・ランクは仮）4.軸別サマリー（強み・弱み・ナレッジ要点・ノルマ計画妥当性要点）5.疑義・要確認（D要点＋C↔D往復の決着）6.変化シグナル（E要点）7.神様的な気づき候補 8.グループ長が見るべき3点 9.最終コメント欄。各項目はどのエージェント由来か明記。
末尾に必ず：「AI補助による月次評価案。最終確定はグループ長が行うこと。あなたは支援AI。」`;
