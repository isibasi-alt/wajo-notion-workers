...
worker.webhook("processProjectLostReportWebhook", {
	title: "WAJO 案件失注報告Webhook",
	description:
		"案件管理DBの営業用「❌ 失注報告する」ボタンから起動。失注理由・失注先・学びの必須3項目が揃っていれば即時失注確定（マネージャー承認なし）。リトライ可能性は任意。失注は評価の宝として AI学習ログにも反映します。",
	execute: async (events, { notion }) => {
		for (const event of events) {
			const body = event.body as Record<string, unknown>;
			...
			await processProjectLostReport(projectPageId, notion as unknown as NotionClient, {
				reason: extractLostReasonFromWebhook(body),
				memo: extractLostMemoFromWebhook(body),
				destination: extractLostDestinationFromWebhook(body),
				learning: extractLostLearningFromWebhook(body),
				retryProbability: extractRetryProbabilityFromWebhook(body),
				triggerUserId: extractTriggerUserIdFromWebhook(body),
			});
		}
	},
...
// === 失注報告（新ボタン）専用オプション ===
// マネージャー承認なし・必須3項目（理由/行き先/学び）で即時失注確定
type LostReportOptions = LostActionOptions & {
	destination?: string;     // 失注先・行き先（必須）
	learning?: string;        // 失注の学び（必須）
	retryProbability?: string; // リトライ可能性（任意：高/中/低）
};

function extractLostDestinationFromWebhook(body: Record<string, unknown>): string | undefined {
	return (
		readNestedString(body, ["失注先・行き先"]) ??
		readNestedString(body, ["data", "失注先・行き先"]) ??
		readNestedString(body, ["properties", "失注先・行き先"])
	);
}
function extractLostLearningFromWebhook(body: Record<string, unknown>): string | undefined {
	return (
		readNestedString(body, ["失注の学び"]) ??
		readNestedString(body, ["data", "失注の学び"]) ??
		readNestedString(body, ["properties", "失注の学び"])
	);
}
function extractRetryProbabilityFromWebhook(body: Record<string, unknown>): string | undefined {
	return (
		readNestedString(body, ["リトライ可能性"]) ??
		readNestedString(body, ["data", "リトライ可能性"]) ??
		readNestedString(body, ["properties", "リトライ可能性"])
	);
}
...
// === 営業用ワンボタン失注報告（マネージャー承認なし・必須3項目で即時失注確定）===
async function processProjectLostReport(
	projectPageId: string,
	notion: NotionClient,
	options: LostReportOptions = {},
): Promise<{ action: string; message: string }> {
	const projectPage = await notion.pages.retrieve({ page_id: projectPageId });
	const projectName = text(projectPage.properties?.["案件名"]) || "案件";
	...
	if (missing.length > 0) {
		await createPageComment(
			...
			projectPage.id,
			[
				`⚠️ 必須項目が未入力のため、失注報告を止めました: ${projectName}`,
				"",
				`未入力: ${missing.join(" / ")}`,
				"失注は評価の宝です。次に活かすため、3項目とも入力してから再実行してください。",
			].join("\n"),
		);
		return {
			action: "needs-required-fields",
			message: `必須未入力（${missing.join("/")}）のため失注確定していません。`,
		};
	}
	...
	await markAiLearningLogsOutcome(notion, {
		...
		outcome: "失注",
		scoreThreshold: 50,
		note: "失注報告Workerが営業の手入力（理由・行き先・学び）を反映。",
	}).catch((error) => {
		console.log("lost report learning outcome skipped", String(error));
	});
